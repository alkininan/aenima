...
describe("getItemByKey", () => {
  ...
  /**
   * §2 lineage. Key and title are embedded rather than looked up separately —
   * it is a to-one from the same row, so it costs no request. The key is what
   * `ItemHeader` links to `/o/<key>` with (T1.4); before it the header could
   * only render the title as plain text.
   */
  it("carries the opportunity the item came out of, by key and title", async () => {
    calls.single = pageRow("soc-12");

    const item = await getItemByKey(WORKSPACE, "soc-12");

    expect(calls.select[0]).toContain("opportunity(key, title)");
    expect(item?.opportunity).toEqual({
      key: "soc-2",
