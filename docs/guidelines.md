<!-- guidelines.md · v1.25 · in the repo · a migration takes one word (T0.26):
     ...
     paragraph says the token only reads from a command line, and the hard boundaries name rule
     (i) and the connector's three other writes where the Backlog Fix used to stand.
     v1.17 · in the repo · §5 step 0 regenerates the route types whenever they
...
A finished ticket merges itself at close and writes the row in the same run; one that weakens a
restraint, or that adds a migration you have not applied, merges on your word, and a merge you make by hand is detected by the next run. You never fill a form.
...
| Review | Done | M | The run's own move at close: the reviewer's PASS on file, the gate green, and a diff that weakens no restraint and adds no migration your `apply` has not already been spent on — merged with a merge commit, Done and the Release row in the same run (§5 step 9). |
...
you have not applied, or a diff that **weakens a restraint**. A migration takes one word and it
is `apply`: the schema is shared and applying one is yours, and the ticket that carries it merges
itself once that word has been spent. Until then the diff waits, because code that reads a column
nobody has created is code main should not carry. `gated.mjs` reads the spending off the task's
own thread and not off anything a run says about it — your reply beginning with `apply`, and the
run's own `applied` note after it naming that migration's tag (T0.26) — so an apply on another
ticket's thread, or for another file, ungates nothing. A weakening is the other half of the same idea — a run must never loosen what a run
...
6  Migration    if the diff adds a migration file: commit, push, stop → Decision (§4). You
                ...
                and the ticket carries on in that same run. That one word is the whole of
                a migration's permission: the ticket merges itself at close (step 9)
...
has read your `merge` there or, on a diff that weakens no restraint and adds no unspent migration, the reviewer's `PASS` on file
