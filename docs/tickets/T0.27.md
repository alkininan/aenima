...
- **Priority** Urgent
- **Type** Technical
- **Spec** Claude Code docs: skills frontmatter `effort` · sub-agents frontmatter `effort` · model-config effort levels
- **Board** https://app.notion.com/p/Extra-high-effort-for-runs-3e379dafd42e81dc9081e9592cf1fc0d

The Spec line cites Claude Code's documentation rather than a repo spec section, so it carries no
version for drift to compare (`version-drift.mjs`: nothing cited, nothing drifted). The three
entries it names are quoted under Cited as fetched on 2026-09-22 from code.claude.com. The one
repo section the Build edits, guidelines §5, is inlined there too.

## Objective

Every pipeline run and every review runs at `xhigh` effort — in Claude Code's words, "deeper
reasoning at higher token spend", one step below max — on the model the routine already uses.
Today a run shows Opus 5 at high, the default, because the scheduled task's form has a model
picker and no effort control.

## Build

- `.claude/skills/ticket/SKILL.md`: add `effort: xhigh` to the frontmatter. Claude Code's skill
  reference: "Effort level when this skill is active. Overrides the session effort level. …
  Options: `low`, `medium`, `high`, `xhigh`, `max`; available levels depend on the model." Every
  `/ticket` run, scheduled or started with Run now, then runs at xhigh for its whole turn.
- `.claude/agents/reviewer.md`: add `effort: xhigh` to the frontmatter — the subagent reference
  has the same field and values. The reviewer's model, fallback and turn cap stay as they are.
- Nothing in settings: an `effortLevel` or `CLAUDE_CODE_EFFORT_LEVEL` in the repository's settings
  would set every session opened in the repository, not only runs.
- `docs/guidelines.md`: one line in the run section naming the effort and where it is set, with
  the version bumped.
...
- **AC1** The `/ticket` skill's frontmatter sets `effort: xhigh`.
- **AC2** The reviewer's frontmatter sets `effort: xhigh`; its model, fallback and turn cap are
  unchanged.

## Tests

- TC1 → AC1 · TC2 → AC2: a unit test parses both frontmatters and asserts the field — and, for the
  reviewer, the unchanged model, fallback and cap — each reddened by removing the field.
  The test is `scripts/run/effort.test.mjs`.
...
Written at step 8 into `docs/reports/T0.27.md` and mirrored into the board row.

## Cited

### Claude Code docs · skills frontmatter `effort` (code.claude.com/docs/en/skills)

> `effort` — Effort level when this skill is active. Overrides the session effort level. Default:
> inherits from session. Options: `low`, `medium`, `high`, `xhigh`, `max`; available levels depend
> on the model.

### Claude Code docs · sub-agents frontmatter `effort`, `model`, `maxTurns` (code.claude.com/docs/en/sub-agents)

> `model` — Model to use: `sonnet`, `opus`, `haiku`, `fable`, a full model ID such as
> `claude-opus-5`, or `inherit`. When you omit it, Claude Code picks the model in the subagent
> model order.
>
> `maxTurns` — Maximum number of agentic turns before the subagent stops. When the subagent reaches
> the limit, Claude Code returns its output marked as partial, and Claude can resume it to
> continue.
>
> `effort` — Effort level when this subagent is active. Overrides the session effort level.
> Default: inherits from session. Options: `low`, `medium`, `high`, `xhigh`, `max`; available
> levels depend on the model.

### Claude Code docs · model-config effort levels (code.claude.com/docs/en/model-config)
