...
**Spec:** docs/guidelines.md §4, §5 · scripts/run/gated.mjs · build-log open question 40 ·
T1.4 and T3.1 reports

# Objective

A migration ticket needs two human words today: *apply*, then *merge*. The second is worse than
redundant. The *apply* is the human's decision about the schema; once it is consumed the shared
database already carries the change, and holding back the code that uses it leaves the database
ahead of main for no reason. T1.4 is the case: `0016` applied from the branch, then the diff sat
gated at Review waiting for a second word about a decision already made.

# Build

1. **A consumed apply ungates its migration.** `gated.mjs` stops treating a migration file as
   gated when the task's thread carries a human *apply* the guard has already verified and a run
   has consumed for that same file — the thread is the record, read by code over the API, never
   the model's account of it. A migration with no consumed apply stays gated exactly as today.
   The loosening detector's other rules are untouched.
2. **Tests of a migration fail on main, skip only on the branch that introduces it.** Build-log
   open question 40, ruled: a database test guarding a schema change may skip while that
   migration is not yet on `origin/main` — that is the branch waiting at Decision for the human's
   *apply*. Once the migration file is on `origin/main`, a missing column is a real failure and
   the test must fail, never skip. Apply to both T3.1's and T1.4's test files, and make the rule
   a shared helper so the next migration ticket does not write its own.
3. Guidelines §4 and §5: a migration takes one human word, *apply*; the merge follows it.

# Rules

- The consumed apply is read from the thread by code, never asserted by the model
- A migration with no human *apply* on its thread is still gated — this ticket narrows the gate,
  it does not remove it
- A skip on main is a failure hidden; the helper must make that impossible, not unlikely

# Criteria

- **AC1** A diff adding a migration whose task thread carries a consumed *apply* for that file
  self-merges on a reviewer PASS
- **AC2** A diff adding a migration with no *apply* on its thread is gated, as today
- **AC3** An *apply* on the thread for a different migration file does not ungate this one
...
- **TC1 → AC1:** `gatedDiff` over a diff of `drizzle/0016_x.sql` and `drizzle/meta/_journal.json`
  with `0016_x` among the thread's consumed applies is `ok: true` with no reasons; and
  `appliedMigrations` reads that tag off a thread carrying the real `migration` → `apply` →
  `applied` sequence. Reddened by dropping the ungating from `gatedDiff`, and by an
  `appliedMigrations` that reads the tag out of the `migration` comment instead of the `applied`
  one.
- **TC2 → AC2:** the same diff with no `apply` on the thread is gated on both files; and a thread
  whose `applied` comment stands *before* the human's `apply` yields no tags — a comment order
  that cannot happen through the guard and must not be read as consumption. Reddened by an
  `appliedMigrations` that ignores the human reply entirely.
- **TC3 → AC3:** a thread whose consumed apply names `0015_y` leaves `drizzle/0016_x.sql` and the
  journal gated. Reddened by matching on "any consumed apply" rather than on the file's tag.
- **TC4 → AC4:** `migrationGate` skips when the file is absent from `origin/main`, does not skip
  when it is on `origin/main`, does not skip when the schema object is present, and does not skip
  when the ref cannot be read at all. Reddened by a gate that skips whenever the object is
  missing.
- **TC5 → AC5:** both `.db.test.ts` files call `migrationGate` and carry no `process.stderr.write`
  banner of their own for the unapplied case; asserted over the files' own source. Reddened by
  restoring either file's inline skip path.

# Done

`pnpm lint && pnpm typecheck && pnpm test`. This ticket narrows the migration gate, which
`loosening.mjs` will read as a weakening — so its own pull request waits for the human's *merge*.
That is correct: a change to what a run may merge on its own is the one change that should.

# Report

See `docs/reports/T0.26.md`.

## Addendum

2026-09-23

> Once T0.36 lands, take main into t0-26: premerge.mjs settles the build log. The guidelines
> header and next-id.test.mjs are yours to settle — put your version line after main's newest
> with the next free number, and keep main's next-id assertion, which pins the rule rather than
> a number. Then land it on the merge already on this thread.

This round is that merge and nothing else: no criterion of the ticket changes, and the five ACs
above are what the merged tree still has to satisfy. What the round owes is a branch that
carries `origin/main`, settled where a script may not settle it.

# Cited

The sections the Spec names, verbatim from the repository at claim.
