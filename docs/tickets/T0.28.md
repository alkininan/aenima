...
**Epic:** E0.2 Pipeline · **Type:** Fix · **Priority:** High
**Spec:** guidelines v1.19 §5 step 1, §7

# Objective

Every task ID names one task. `scripts/run/next-id.mjs` numbers a task within its own epic, so a
claim in an epic with no numbered task yet, or in one whose next number another epic has since
used, answers an ID that already belongs to another task: E0.4 Design conformance answered T0.1,
the scaffold ticket's, until T0.40 was given by hand, and E0.1 Foundation answers T0.7, E0.2's
Setup. The ID names the task's branch and its files on main, so a repeat overwrites another task's
record. R-0062 stopped at claim on the first of these.

# Build

- `next-id` answers the lowest number in the task's phase that is not taken. Taken means a task on
  the board holds it, whatever its epic or status, or a branch or a file under `docs/` carries it.
  The epic still gives the phase; it no longer gives the count.
- T0.96 and T0.40, the number E0.4's first task was given by hand, are taken numbers like any
  other. The count steps over them, and no epic owns a block.
- `docs/guidelines.md` §7 Names states the rule in one sentence. Bump its version.

# Rules

- As the repository's: `CLAUDE.md`, `AGENTS.md`, `docs/guidelines.md`.
- No existing ID changes.
- A number is never reused: one that a branch or a docs file still carries stays taken after its
  task is gone.

# Criteria

- **AC1** A claim in an epic with no numbered task answers the lowest free number in its phase:
  with T0.1 to T0.27 and T0.96 taken, T0.28.
- **AC2** A claim in an epic whose next number another epic holds skips it: with E0.1 at T0.1 to
  T0.6 and E0.2 at T0.7 to T0.27, an E0.1 claim answers T0.28, not T0.7.
- **AC3** A number carried only by a branch or a docs file, with no task on the board, is not
  answered.
- **AC4** `docs/guidelines.md` §7 states the rule, and its version is bumped.

# Tests

- **TC1 → AC1:** `next-id` over fixture rows answers T0.28 for a task in a new epic; red against
  the per-epic count, which answers T0.1.
- **TC2 → AC2:** the same fixture with an E0.1 task answers T0.28; red against the per-epic count,
  which answers T0.7.
- **TC3 → AC3:** with T0.28 carried only by `docs/log/T0.28.md`, `next-id` answers T0.29.
- **TC4 → AC4:** grep `docs/guidelines.md` for the rule under §7 and the new version in its header.
...
_Written at step 8 into `docs/reports/T0.28.md`._


## Cited

Every section this ticket cites, verbatim from the repo at claim.
