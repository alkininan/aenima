...
> Once T0.36 lands, take main into t0-34: premerge.mjs settles the build log. The guidelines header and next-id.test.mjs are yours to settle — put your version line after main's newest with the next free number, and keep main's next-id assertion, which pins the rule rather than a number. Then land it on the merge already on this thread.
