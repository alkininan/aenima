...
- **Priority** Urgent
- **Type** Technical
- **Spec** docs/guidelines.md §4, §5 · T0.10's capability boundary · T3.1 and T1.4 migration stops
- **Board** https://app.notion.com/p/T0-24-Apply-from-a-worktree-run-3e179dafd42e81fdafddf69031bc4f1a

## Objective

A granted *apply* can never be consumed. Scheduled runs happen in worktrees; `.env.migrate` lives
only in the primary checkout by design (T0.10's credential boundary); so every run that reads an
*apply* has to leave it unconsumed and hand it to "a run in the primary checkout" — which the
scheduler never starts. T3.1's *apply* has sat granted since 2026-09-20 with nothing able to act on
it, and every migration ticket after it will deadlock the same way.

## Build

The boundary is right and stays: a pipeline session must not hold a credential that can change
schema. What is wrong is that nothing can act on the human's word. Pick the smallest fix that keeps
the boundary and unblocks the word — candidates, in the order I would try them:

1. **A primary-checkout step in preflight.** The run stays in its worktree for everything else;
   when it reads a granted *apply*, it runs `pnpm db:migrate` in `~/dev/aenima` through a narrow
   script that does nothing else, reads `.env.migrate` from there, and reports what was applied.
   The credential never enters the worktree; the run never holds it.
2. **A migration queue.** The run records the granted *apply* against the migration file; a separate
   scheduled routine in the primary checkout applies it and comments. Two routines, one of which
   only ever migrates.
3. **Reject the premise** — if neither of the above can be done without widening the boundary, say
   so and the human keeps applying by hand, but the skill must then stop claiming a run in the
   primary checkout will pick it up, and must tell the human plainly that the terminal step is
   theirs.

Whichever lands: the *apply* is consumed exactly once, the applied migration is named in the
report, and the ticket continues in the same run rather than waiting for another tick.

## Rules

- The credential boundary does not move: no pipeline session in a worktree reads `.env.migrate`
- A granted word is consumed once, by the run that acts on it — never re-asked and never silently
  dropped
- If the fix cannot keep the boundary, it is not the fix; say so and change the skill's wording
  instead

## Criteria

- **AC1** A granted *apply* on a ticket with an unapplied migration results in that migration being
  applied, from a scheduled run in a worktree
- **AC2** The worktree session never reads `.env.migrate` — asserted, not assumed
- **AC3** The *apply* is consumed once: a second run finds no outstanding word
- **AC4** The report names the migration applied and its journal index
- **AC5** A failed apply stops the ticket at Decision with the database's own error quoted, and does
  not consume the word
...
`pnpm lint && pnpm typecheck && pnpm test`; T3.1's outstanding *apply* observed consumed end to end

## Report

Written at step 8 into `docs/reports/T0.24.md` and mirrored into the board row.
