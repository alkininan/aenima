...
Claimed by the scheduled run on 2026-09-22 (session `d1bb8fef`) in its own worktree · Epic E0.2
Pipeline · Priority Urgent · Type Fix · Blockers T0.17 Linear ordering (Done) · Spec — none cited,
so nothing to drift against; built against guidelines v1.17 as main holds it. Branch `t0-18` off
`origin/main` at `11c946a`. Board row:
https://app.notion.com/p/T0-18-Guard-the-token-3dc79dafd42e812dbc85c256f5027351. The body below
is the board's, verbatim — drafted by the pipeline on 2026-09-15 from T0.17's review and readied
by the human's `ready` on 2026-09-22. Its Rules name the repository's CLAUDE.md, AGENTS.md and
`docs/guidelines.md`; the Cited section holds the two guidelines sections the routes live in, §4
(the three words verified in code, the guard at the connector) and §5 (the board's token, the
hard boundaries), from the repo at v1.17. T0.17's own record of the routes is
`docs/reviews/T0.17.md` (pass 3, Should 2) and `docs/reports/T0.17.md` (open question 1).

## Objective

T0.17's reviewer, pass 1, Should 7 — found while building ready as a word the guard reads at the
connector; the fixture reply posted with the token came back with the human's user id. Close the
routes by which a run can still author the human's word or skip Backlog: a comment posted over the
Notion API with NOTION_TOKEN from a Bash script (node or curl) lands authored as the human's own
user and is read as their merge, apply or ready, a status write sent over the API with the same
token (PATCH /pages) sets a Backlog task Ready without passing the connector the guard stands at, a
Ready task duplicated or moved into Tasks through the connector keeps its status (T0.17's reviewer,
pass 2, Should 3), and the connector's notion-update-data-source could rename the Status options
under every task at once (pass 3, Should 2). Refuse or detect each in the guard, with a test per
route.
...
- AC1 Close the routes by which a run can still author the human's word or skip Backlog: a comment
  posted over the Notion API with NOTION_TOKEN from a Bash script (node or curl) lands authored as
  the human's own user and is read as their merge, apply or ready, a status write sent over the
  API with the same token (PATCH /pages) sets a Backlog task Ready without passing the connector
  the guard stands at, a Ready task duplicated or moved into Tasks through the connector keeps its
  status (T0.17's reviewer, pass 2, Should 3), and the connector's notion-update-data-source could
  rename the Status options under every task at once (pass 3, Should 2). Refuse or detect each in
  the guard, with a test per route.
...
Written by the run at close — `docs/reports/T0.18.md`, mirrored into the board row.

## Cited

`docs/guidelines.md` §4 and §5 at v1.17, verbatim from `scripts/run/spec-sections.mjs`.
