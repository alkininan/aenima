...
2026-09-15 · reply on the board, at Review, verbatim:

> default

It answers the pipeline's comment of 2026-09-15 11:09 UTC, which stopped on the human's earlier
`merge` because the pull request no longer merged cleanly into main — T0.16 had rewritten the
same passages (the guidelines' version header and §5 steps 0 and 9, the /ticket skill's steps 0e
and 9, the run scripts' README) — and offered: *"If you say "default" I'll merge main into the
branch, keep T0.16's self-merge and deploy check and fold this ticket's mirror and Runs steps in
beside them, have it reviewed again, and bring it back to Review for a fresh merge from you."*
Since then T0.17 (Linear ordering) has landed on main too (`11dc6e3`), touching the same files,
so "main" is `origin/main` at `11dc6e3`: guidelines v1.9, T0.16's self-merge and deploy check,
T0.17's picker, `ready` word and connector guard.

This round builds the reply read as criteria:

- **AA1** `t0-12` merges into `origin/main` without conflict, and every line T0.16 and T0.17 put
  in the four files that conflicted — `.claude/skills/ticket/SKILL.md`, `scripts/run/README.md`,
  `docs/guidelines.md`, `docs/build-log.md` — is still there, except the placeholders that said
  the mirror and the Runs row were a later ticket's, which this ticket's text replaces.
- **AA2** The mirror refresh sits beside the deploy check, after it — skill step 0f after 0e,
  the README's preflight paragraph, guidelines §5 step 0 — so a revert the deploy check pushed is
  what the mirrors reflect.
- **AA3** The Runs row sits beside the self-merge in step 9 (skill, README, guidelines §5 step 9
  and §2 Runs), and `runs.mjs` reads the protocol main now runs: a run that merged its own work
  at close (Review then Done) is Done, and the row names the run's own task when step 0 claimed
  and released another task to merge or apply before step 1 claimed it — as this very session
  did (T0.17 merged, T0.13 refused, then T0.12 claimed).
- **AA4** The guidelines header names this change as v1.10 above v1.9 and v1.8; §8 names T0.11
  v1.6, T0.16 v1.8, T0.17 v1.9 and T0.12 as this version; the build log's list is regenerated
  from `docs/log/`, not edited.

Tests:

- TA1 → AA1 · `git merge-tree --write-tree origin/main t0-12` reports no conflict; the whole
  suite green at the pushed tree (the Stop gate); the reviewer reads
  `git diff origin/main...t0-12 -- .claude/skills/ticket/SKILL.md scripts/run/README.md docs/guidelines.md`
  for anything of T0.16's or T0.17's lost.
- TA2 → AA2 · read in the same diff; `scripts/run/readme.test.mjs` for the README's shape.
- TA3 → AA3 · `scripts/run/runs.test.mjs`, the block tagged `TA3 → AA3`: "takes step 1's claim as
  the run's, not a task step 0 claimed to merge and released", "reads a run that only merged in
  step 0 and claimed nothing after as Stopped, no task", "keeps a claim that no Status write
  followed as the run's while it still stands", "reads a run that merged its own work at close as
  Done", "reads a stop that released the marker before setting Decision as Decision, with its
  task", "does not read gh pr merge inside a heredoc body as the claim's merge", "takes step 1's
  claim over a step-0 claim that did not merge, both in one command", "keeps a killed run's claim
  over a later command that only quotes one"; and, above that block, "does not read a
  claim.mjs quoted in a later command's prompt as a claim" (renamed from the first round's "takes
  the first claim as the run's…").
- TA4 → AA4 · `scripts/run/log-index.test.mjs` refuses a stale list; the header and §8 read in the
  diff.

Cited for this round: `origin/main` at `11dc6e3` is the text AA1 keeps — read it with
`git show origin/main:<path>`; the sections cited above are the repo's as merged (guidelines v1.10).
