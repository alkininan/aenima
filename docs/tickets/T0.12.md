...
- TA3 → AA3 · `scripts/run/runs.test.mjs`: "takes step 1's claim as the run's, not a task step 0
  claimed to merge and released", "reads a run that only merged in step 0 and claimed nothing
  after as Stopped, no task", "keeps a claim that no Status write followed as the run's while it
  still stands", "reads a run that merged its own work at close as Done", "ends a claim at
  release.mjs run as a command, not at a command that names the file"; and the renamed "keeps the
  claim that wrote a Status over a later command that mentions one".
