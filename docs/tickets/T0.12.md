...
- TA3 → AA3 · `scripts/run/runs.test.mjs`, the block tagged `TA3 → AA3`: "takes step 1's claim as
  ...
  task", "does not read gh pr merge inside a heredoc body as the claim's merge", "keeps a killed
  run's claim over a later command that only quotes one"; and, above that block, "does not read a
