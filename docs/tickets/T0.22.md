...
Claimed by the scheduled run on 2026-09-16 (session `998a6de8`) in its own worktree · Epic E0.2
Pipeline · Priority Medium · Type Technical · Spec `docs/guidelines.md v1.12 §5 ·
.claude/settings.json fallbackModel · T0.19 report`. Branch `t0-22` off `origin/main` at
`41e2d0f`. Board row:
https://app.notion.com/p/T0-22-Reviewer-model-fallback-3dd79dafd42e8142887ae7288a06301e. The body
below is the board's, verbatim. The Cited section holds guidelines §5 from the repo; the two other
things the Spec line names are not spec sections and are read where they live —
`.claude/settings.json` (`"fallbackModel": ["opus"]`) and `docs/reports/T0.19.md` (the *Re-close
round* paragraph and the first item of its re-close Open questions, where the override was first
reported).

**Changed since this ticket was cut:** the Spec line cites guidelines **v1.12**; the repo holds
**v1.13** (T0.13's — the production database role, §5's capability boundary). The Build's last line
asks for "Guidelines v1.13, one header line"; v1.13 is taken on `main`, so this ticket's header line
is **v1.14**. `version-drift.mjs` reads the document as `guidelines`, not `docs/guidelines.md`.

## Objective

The reviewer is configured to run on Fable. Fable has been out of weekly credits for days, so every
run has overridden to Opus and said so in its report — the right call each time, but nothing in §5
says a run may make it. Write the rule down so a run follows it rather than improvises it.

## Build

- Guidelines §5 step 5: if the reviewer's configured model refuses the call for credits or
  availability, the run falls back to the next model in `.claude/settings.json`'s `fallbackModel`
  and names the model it used in the report. The review still runs in a fresh session with no
  briefing — only the model changes.
- The report's reviewer section names the model for each pass, so a reader can see which mind
  passed which code.
- Nothing else changes: the three-pass cap, Must/Should tagging and the PASS verdict file are as
  they are.
- Guidelines v1.13, one header line.

## Rules

- The fallback is the configured chain, never a model the run picks for itself
- A review that could not run at all is a stop, not a pass — the ticket waits rather than closing
  unreviewed

## Criteria

- AC1 §5 step 5 states the fallback and the reporting duty
- AC2 The report template names the model per pass
- AC3 A reviewer call that fails for a reason other than credits or availability still stops the
  run
...
Written at close — `docs/reports/T0.22.md`.
