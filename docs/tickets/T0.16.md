...
Claimed by the scheduled run on 2026-09-14 (session `9d508dfb`) in its own worktree · Epic E0.2
Pipeline · Priority High (the board's own scale; `pick-next.mjs` reads Must/Should/Could and ranks
it last, oldest first) · Type Technical · Spec `docs/guidelines.md v1.7 §3–§5 · T0.11 report · the
three "Read Status as select" rows` — **drifted**: the repo holds guidelines v1.6 and no v1.7 exists
on main; the ticket's own item 6 takes it to v1.8, so this is built against v1.6 as the record.
Branch `t0-16` off `origin/main` at `0870c08`. Board row:
https://app.notion.com/3db79dafd42e81538a9bdcffdfc70538. The body below is the board's, verbatim;
the Cited section holds every cited section from the repo at v1.6, plus what the two other
citations are.

## Objective

A finished ticket merges itself. The human's word *merge* is required only where a human still
adds something: a migration (their *apply*), a product-spec change (a decision, not code), or a
change to the pipeline's own guard, gate, skill or run scripts (a run must never loosen its own
boundary unattended). Everything else lands on main and deploys when the reviewer has passed and
the gate is green. This also carries the Status fix every run has been filing.

## Build

1. **Status as a select.** `scripts/run/notion.mjs` and everything downstream read
   `props.Status.select.name`; the board's Status is a select, not a status type. Every merge and
   apply has been refused on this since T0.11. Test with a fixture row of each shape.
2. **Self-merge at close.** Step 9: after reviewer PASS (no open Must) and a green gate, if the
   diff touches none of the gated paths, `gh pr merge --merge --delete-branch`, then Done and
   the Release row in the same run — no waiting for the next tick. Gated paths: `drizzle/**`,
   `docs/product-spec.md`, `.claude/**`, `scripts/hooks/**`, `scripts/run/**`,
   `.worktreeinclude`, `.gitignore`, `package.json` scripts. A gated diff stays at Review and
   posts one plain comment saying which path is waiting for the word.
3. **The guard's merge rule** grows a second door: `gh pr merge` is also allowed when the
   marker's task has a reviewer verdict file `docs/reviews/<id>.md` ending in PASS and the diff
   against origin/main touches no gated path — both checked by the guard in code. The
   human-comment door stays for gated diffs.
4. **Hooks run the main copy.** The guard and gate commands in `.claude/settings.json` execute
   `git show origin/main:scripts/hooks/<name>.mjs`, not the worktree file, so a run editing its
   own guard changes nothing until a human merges it. Test: edit guard.mjs in a temp worktree to
   allow everything; a refused command is still refused.
5. **Post-merge check and revert.** Preflight step 0: if main advanced since the last Release
   row, check aeni.ma from outside (`/sign-in` 200, `/app` 307). On failure: `git revert -m 1`
   the merge commit, push main, file one Fix task at Backlog naming the reverted ticket, and post
   a plain comment on that ticket. The reverted ticket returns to Backlog.
6. Guidelines v1.8: §3 Review → Done is the run's own move except on gated paths; §4 the three
   cases where *merge* is still the human's word; §5 hooks from main, the outside check and
   revert.

## Rules

- The reviewer's PASS is a file the guard reads, not a claim the model makes in the transcript
- A run can never merge a diff that changes what a run is allowed to do
- Revert is a merge commit reverted, never a force-push

## Criteria

- AC1 A fixture at Review with the word *merge* is now merged (the Status fix)
- AC2 A non-gated ticket reaches Done and a Release row in the run that built it, with no human
  comment
- AC3 A diff touching `scripts/hooks/` stays at Review with the comment naming the path, and
  merges only after the word
- AC4 A guard edited to allow-all in a worktree still refuses `db:push`
- AC5 A deliberately broken sign-in merged on a fixture is reverted by the next run, with the Fix
  row and the comment

## Tests

- TC1 → AC1 · TC2 → AC2 · TC3 → AC3 · TC4 → AC4 · TC5 → AC5 (fixtures, deleted after; a preview
  deploy is fine for TC5 if production is not — say which)
- Files: `scripts/run/notion.test.mjs`, `scripts/run/threads.test.mjs` (TC1);
  `scripts/run/gated.test.mjs`, `scripts/run/permission.test.mjs`, `scripts/hooks/guard.test.mjs`,
  `scripts/run/comments.test.mjs` (TC2, TC3); `scripts/hooks/hooks.test.mjs` (TC4);
  `scripts/run/health.test.mjs`, `scripts/run/revert.test.mjs`, `scripts/hooks/guard.test.mjs`
  (TC5)

## Done

`pnpm lint && pnpm typecheck && pnpm test`; TC1–TC5 observed. This ticket's own PR is gated (it
touches the harness): it waits for the human's *merge* — the last one.

## Report

Written by the run at close — `docs/reports/T0.16.md`, mirrored into the board row.

## Cited

The Spec line cites three things. `docs/guidelines.md v1.7 §3–§5`: no v1.7 exists in the repo
or on any branch merged to main; the sections below are v1.6's, which is what main holds and
what this ticket changes. `T0.11 report`: `docs/reports/T0.11.md` in the repo, the ticket that
built the two words and the guard's reading of them. `The three "Read Status as select" rows`:
Backlog tasks on the board — *Read board Status as select* (3db79daf-d42e-81a1-b0f7-e8e5199ca410),
*Read Status as a select property* (3db79daf-d42e-8125-8249-cd963d2a25a1) and *T0.15 Read the
board's select Status* (3db79daf-d42e-81e2-a095-d299e9cbc941, branch `t0-15` on origin at
`f0b4960`, whose fix this ticket carries) — each saying the same thing: `task()` in
`scripts/run/notion.mjs` reads `props.Status.status.name` and the board holds a select, so every
row reads at no status and the guard refuses every merge and apply. A fourth, *Guidelines §2
Status type is select* (3db79daf-d42e-8183-a7e6-f7455eb98215), came from T0.15's reviewer and
asks for the one cell in §2; item 6's v1.8 pass carries it.
