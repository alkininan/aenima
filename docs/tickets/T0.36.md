...
Spec: guidelines v1.20 §5 steps 0 and 9 · Epic: E0.2 Pipeline · Type: Fix · Priority: High

# Objective
A ticket that waits at Review cannot merge once anything else has. Since T0.32, `docs/build-log.md` carries a section generated from `docs/log/`, and every branch regenerates it, so two branches never agree on it. GitHub refused T0.26's and T0.34's merges on that file alone, and T0.30's is next. Self-merged tickets escape only because nothing lands between their branch and their merge. The T3.2 run already showed the resolution by hand: merge main in, take main's copy of the generated section, regenerate. Until the merge step does that itself, every `merge` word given to a waiting ticket is refused, and the refusal comment's "the next run makes it again once the branch merges cleanly" never comes true.

# Build
- Before any merge — a `merge` word assessed at step 0, and the self-merge at step 9 — merge `origin/main` into the branch with a merge commit.
- A conflict confined to generated files (the build log's generated sections and the log index) resolves by taking main's copy and rerunning `scripts/run/log-index.mjs`. The gate then runs on the result, the branch is pushed, and the pull request merges.
- Any other conflict is refused as now, naming the files, and the task stays at Review.
- `docs/guidelines.md` §5 says so at steps 0 and 9. Bump its version to a number no open branch holds.

# Rules
- As the repository's: `CLAUDE.md`, `AGENTS.md`, `docs/guidelines.md`.
- A merge commit, never a rebase: the guard refuses the force-push a rebase needs.
- A generated file is regenerated, never hand-resolved.

# Criteria
- AC1 A branch whose only conflict with main is in generated files merges: main is merged in, the sections regenerated, the gate green on the result.
- AC2 A conflict in any other file is refused, names the file, and pushes nothing.
- AC3 Guidelines §5 states the step at 0 and 9, and its version is bumped.

# Tests
- TC1 → AC1: a fixture repository with two branches that each regenerate the build log; merging the second after the first succeeds, and the generated section equals a fresh regeneration. Red without the step.
- TC2 → AC2: the same fixture with a conflicting line in another file is refused naming that file, and the branch head is unchanged.
- TC3 → AC3: grep for the step under §5 and the new version in the header.
...
> T3.2 lands ahead of you on its merge, which turns main's gate green and leaves its build-log entry as your only conflict. Take main into t0-36 with this branch's own premerge.mjs, run the gate, and land it: the PASS on file stands, since merging main in changes nothing of yours.
