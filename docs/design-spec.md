# aenima — Design Specification v2.10 (web)
...
**Resend cooldown.** A tertiary resend action ("Send a new code") disables itself for 60s after each use and counts down in its own label — "Send a new code (0:47)" — returning to its normal label at zero. The cooldown is the mechanism, not advice: without it a second tap hits the provider's rate limit and the failure lands on whatever field is on screen, which blames the user for a control the product left enabled. A control that cannot succeed yet is disabled, never merely apologised for.
...
*v2.10 — complete and closed: no open items. Changes cut new versions of this document.*
