# aenima — Design Specification v2.17 (web)
...
v2.10: two values the v2.8 implementation derived become law, and the resend gets the control it was missing. The step title is named display-lg with its subtitle in the §4 slot — the two references the v2.7 rewrite dropped when it replaced that paragraph (§8). The at-rest label offsets 9 rather than 8: it sits inside the pill's 1px border, so the border counts, and label and value paint at the same x (§8). A tertiary resend disables itself for 60s and counts down in its own label (§8) — without it a second tap hits the provider's rate limit, and that failure was landing on the OTP field, so a helper line is named as carrying only errors about its own field's value and a control's failure surfaces at that control (§8). The rate-limit string stops implying the person was excessive when the product had left the button live (§12).
...
v2.16: the item page's expansion becomes law — the check list, the disclosure as a component, and §5's moves on a gap (§8). The check list is the whole rubric in pack order with three line states: passed as a mono-micro `--success` label with a 12 tick, unclear as the open gap chip over a `--surface-1` evidence card, not asked as a mono-micro `--n-secondary` label with the condition negated beneath it. v2.15's item-row rule holds one level down — a container means a gap, so only the unclear line carries a chip — and **not asked is deliberately not the excluded chip**: exclusion is a negotiation outcome with a person's name on it, non-applicability is arithmetic. The disclosure is native `<details>`/`<summary>` with `.control` on the summary for §7's states, `control-edge-none`, marker removed in both spellings, and a 16 chevron swapped rather than rotated; native because it costs no client island, survives JavaScript being off, and gets §11's keyboard path right unaided. Gap moves: accept inside a disclosure over §8's 48h field and a Primary md submit, reopen as a standing **Secondary** sm rather than Ghost (a text-only control on a .60 card vanishes where it has most to say), no confirm step on either since neither destroys anything, and Danger on nothing but the reason field's helper line. And the rule a review forced: a move's confirmation renders **outside** the disclosure its own outcome closes.
v2.17: two values in the item page's expansion become law rather than staying invisible in a component (§8). The meter block is bounded at **400**, the confirm modal's own max and the same job — a column that should not run to the full width of a page; it was a hand-picked 420 in the file whose work is rendering law, and the unscored state carries the same bound so the track does not move. The expanded list below it is named as **not** separately bounded: a check's wording and a quoted evidence card are prose, and §3's reading measure governs them, not the meter's column. And the passed line's 12 tick is pinned to the sortable-header chevron's precedent rather than to mono-micro's 14 line box, so nobody re-derives it from the type scale and "corrects" it to 10.
...
- **Sidebar.** `--bg-base`; **a column of three zones — lockup, nav, account.** Lockup top: Æ mark 24px + `aenima` wordmark; product switcher directly beneath it = avatar 40 + display-md; nav items 40px (icon 20 + ui-body), active = `--prime-soft` pill + `--n-primary`. **The account slot is pinned to the bottom:** avatar 32 + the signed-in address in ui-body, truncating. Identity is something you check and change, not something you navigate between, and the top-left position belongs to the things that move you around — the switcher stays at the top because it filters the list rather than identifying the person. Sidebar never collapses in v1; the chat dock is what collapses.
...
**Inputs.** Field 48h, `--r-pill`, `--surface-1` fill overlaid with `--sheen` beneath the top specular, 1px `--glass-border`, pad 16 horizontal (height wins: the value centres vertically, per the §8 option-row rule), icon slots 24 leading/trailing, gap 8, ui-input text. 48 aligns fields with the lg button, so a field and its submit sit at one height. Focus: pointer → `--prime` border only; keyboard modality (§6) → border + ring + glow. Error: `--danger` border. Disabled: 40% opacity. **The leading icon takes the field's state colour** — `--prime` on focus, `--danger` on error, `--n-secondary` otherwise, `--n-disabled` when disabled. The icon is part of the field's state, not decoration beside it; a field that signals only at its border makes the user hunt for what changed. **No `backdrop-filter` on a field:** the sheen is a material, not a layer, and blur is reserved for genuine layer overlap — dock, modals, topbars, toasts (§4).
**Floating label.** The label is a real `<label>`, always in the DOM, never a placeholder. At rest (empty, unfocused) it renders inside the field where the value will sit — ui-input size, `--n-secondary` (AA), not placeholder tone. The at-rest label sits inside the pill's 1px border, so its rest offset is 9 (41 behind a leading icon): label and value paint at the same x, and the border is part of the geometry, not a rounding error. On focus or once filled, it floats to the label zone above the field: ui-label (§3), `--n-secondary`, 6 above the field, left-aligned 8 from the field's left edge, in a label zone with 8 side padding — the label hugs the container edge regardless of leading icons; only the value moves for an icon. The float animates translateY + size over `--t-fast` `--ease`; `prefers-reduced-motion` snaps it. The label zone (24h = 18 line + 6 gap) is **always reserved**, so floating never shifts layout. A field shows one text, ever: its label. Format-hint placeholders are abolished — after the label floats, the field is empty with a caret. Nothing swaps, nothing appears. **Exempt from the floating label:** Search (the leading icon names it; placeholder "Search" allowed at rest) and the chat composer (the dock names it) — both are labelled by context, not by a `<label>` row. `--n-placeholder` exists only for these two.
**Helper line speaks only in states.** The helper slot (ui-footnote, 8 below the field) carries validation outcomes exclusively — error `--danger`, warning `--warning`, success `--success` — never instructions; instructional copy lives in the subtitle slot (§4). **A field's helper line carries only errors about that field's own value.** A failure belonging to a control elsewhere on the step surfaces at that control, never on the nearest field. Forms **reserve one helper line** (18h) under any field that can produce a state, so an error appearing never shifts layout. **Validation timing — flag slow, clear fast.** A field never errors while empty or under 3 characters, including on blur: leaving an untouched field is not a mistake. While typing, validation waits for a 1.5s pause; blur validates immediately once the field carries ≥3 characters; on submit, everything validates and the first error is scrolled to. The moment input becomes valid, the error clears instantly — flagging waits, clearing never does.
...
**Multi-step flows (sign-in, onboarding, ceremony steps).** The Æ mark is centered above every step. The step title is display-lg (§3) and its subtitle is the §4 subtitle slot. A step with no back button centers its title and subtitle beneath the mark. A step with a back button uses the header grammar instead: a row of back control then title, with the subtitle under the title — title and subtitle left-aligned to each other, never centered beside a back control. The back is an IconButton, neutral variant (visible `--surface-2` circular fill at rest — ghost disappears in a header), 48, arrow-left 24, gap 12 to the title block. Back always means "previous step" and replaces textual escape hatches that mean the same thing ("Use a different email" is the back button). Controls follow, then the primary filling the content width; tertiary actions ("Send a new code") beneath it, centered, neutral, maximum one per step. **Back and the tertiary use the same variant:** they are the same tier of chrome on the same step and must read as one family. Ghost is wrong for a tertiary that can be disabled and counting down — it vanishes precisely when it has the most to say. The OTP group stays centered (six discrete boxes are a display, not a text field). Step changes are instant — no slide transitions in v1.

**Resend cooldown.** A tertiary resend action ("Send a new code") is disabled for 60s after a code is sent and counts down in its own label — "Send a new code (0:47)" — returning to its normal label at zero. The cooldown starts when a code is sent, not when the resend is pressed: arriving at a code step means one has just gone out, so the resend opens already counting down. The provider's window belongs to the address, not to the control, and a resend that is live on arrival hands the user exactly the rate-limited tap the cooldown exists to prevent. The cooldown is the mechanism, not advice: without it a second tap hits the provider's rate limit and the failure lands on whatever field is on screen, which blames the user for a control the product left enabled. A control that cannot succeed yet is disabled, never merely apologised for.

**Select/dropdown.** Trigger = pill field with trailing chevron (20) — it inherits the full input grammar, 48h and floating label included. Panel: `--surface-1`, radius 12, dropdown shadow, 6px padding; options 36h, ui-body, pad 12 horizontal, radius 6 (§5's nested rule: 12 − 6); hover `--surface-3`; selected `--prime-soft` + check icon 16. **Height wins:** 36 comes from §4 density and governs the whole system; the label centres within it rather than adding vertical padding. Opens below (above if <320px space), 8 from the trigger; max-height 320 with inner scroll; type-to-jump.
...
**Gap moves.** §5's negotiation moves on a gap, wherever that gap is drawn — the card and the check list render the same control, never two.
- **The settled stamp** is ui-footnote `--n-secondary`: who accepted or excluded it, and their reason after an em dash. §1 law 7 makes a debt something a *named* person owns, so the name travels with the gap rather than living only on the card.
- **Accept** is a disclosure (above) with a ui-footnote `--n-secondary` summary, pad 8/4, gap 4. Inside it, §8's field grammar unchanged — 48h pill, floating label, one reserved 18h helper line — and a **Primary md** submit.
- **Reopen** is a standing control beside the stamp, **Secondary sm and not Ghost**: an accepted card already renders at opacity .60, and §8's own warning about a text-only control that "vanishes precisely when it has the most to say" is exactly this case. §0 law 7 dims it; nothing disables it.
...
- **Unclear** — the §8 gap chip in its open tones: Must `--warning-soft` bg + `--warning` text, Should `--surface-2` + `--n-secondary`. Only a Must is warm, because only a Must blocks. The quoted evidence sits under the line on a `--surface-1` card in ui-body.
- **Not asked** — mono-micro `--n-secondary`, no container, with the condition beneath it in ui-footnote `--n-secondary`. §4 writes the condition affirmatively and the check is not asked precisely because it is false of the artifact, so the negation belongs to the frame around the condition, never to the condition itself.

**The meter block is bounded at 400** — the summary row carrying the meter, the mono-readout percentage and the freshness, and the unscored state at the same width so the track sits in one place whether or not there is a run behind it. §8 already names 400 as the confirm modal's max and the job is the same: a column that should not run to the full width of a page. **The expanded list below it takes the page column and is not separately bounded** — its measure is §3's reading rule, since a check's wording and a quoted evidence card are prose and a 400 column would wrap every line of it.

**A container means a gap, one level down as well.** Only the unclear line carries a chip; a pass and a not-asked check carry text labels without one. This is v2.15's item-row rule ("a bordered chip in a row must mean something, and what it means there is a gap") applied inside the expansion. **Not asked is deliberately not the excluded chip** even though both describe a check that left the denominator: exclusion is a negotiation outcome with a person's name on it (§5's first move), non-applicability is arithmetic the engine did. Giving them one treatment would make the machine's bookkeeping look like somebody's decision. **No Danger on this list at all** (§0 law 1, §12): an unclear Must is `--warning`, an unclear Should is neutral, and nothing here is red.

**Disclosure.** Native `<details>`/`<summary>` wherever a surface expands in place. The summary wears `.control`, so §7's states arrive whole — hover overlay, press physics, and the focus ring *with* the `--prime-glow` §6 and §7 pair it with — plus `control-edge-none`, since the specular edge is Primary's alone. Radius `--r-sm`, pad 8. The browser marker is removed in both spellings (standards and WebKit): the affordance is the chevron, and a triangle beside it would be two. Chevron 16, **swapped rather than rotated** — chevron-right closed, chevron-down open — because §6 names no duration for a disclosure and v1 makes step changes instant. **Native, not a component:** the element already holds the only state a disclosure has, so this costs no client island, works with JavaScript off, and gets §11's keyboard path (Tab, Enter or Space) and screen-reader semantics correct without help. A disclosure that would open onto nothing is not rendered at all (§10).
