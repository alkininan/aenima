...
v1.5: Three rulings the build produced. A scoring run stamps **protocol version** beside provider, model and rubric version, and the protocol joins the cache key — it is the instruction layer around a rubric, it changes verdicts, and its version is computed from the assembled context rather than declared, so an edit to any renderer moves it (§5). A run **stores** the checks that left its denominator and the condition that excluded each, because re-deriving them from the current pack lets a rubric edit rewrite what an old run says it did not ask, and the lines under a stored denominator stop adding up to it — §1 law 3's failure where a reader goes to interrogate a number (§4). And the Decider appointment **does not override the role table**: a Decider settles a blocking gap whatever their role except a Viewer, since read-only means read-only and a per-product field must not grant a workspace-level write — naming a Viewer as Decider is refused at assignment time rather than honoured and then ignored (§14).
...
- The same engine governs individual checks: "offline behavior" applies to a network-dependent mobile screen, not to an admin dashboard. Conditions are evaluated by the agent in the same pass that scores; non-applicable checks leave the denominator. A condition may carry probes the way a check does (§5) — questions answered from the artifact before the condition is decided — so that the same document brings in the same checks on every run; a probed condition is decided by its probes and by nothing else.
...
The author slices the converged PRD into stories with GWT acceptance criteria, each traced to a requirement ID. The critic checks the slice: no story too big, no requirement orphaned, no acceptance criterion untestable, and **build order** — no task scheduled before its dependency. The no-orphan rule runs one level down as well, inside the ticket: every acceptance criterion names the test case that verifies it, and every test case names its criterion, before the ticket is pushed. No orphan in either direction. The owner walks the slice conversationally (merge, split, kill). Output: the sprint-ready backlog, pushed to the task manager at sign-off.
...
The final deliverable: everything the ceremony validated, formatted for the coding agents that will build it. Emitted as Spec Kit–compatible markdown (`spec.md` / `plan.md` / `tasks.md`) with the constitution as `AGENTS.md` (the Linux Foundation cross-tool standard). English by default. Design law: **each ticket's pack is self-contained** — everything needed inlined, everything else excluded.

**Delivery.** aenima never rewrites, moves or appends to an instruction file the team owns — their `CLAUDE.md`, their `AGENTS.md`, their agent's own rules. It delivers through exactly one channel per repository, never a copy beside the file it copies: an instruction file loaded twice is context spent twice, and two copies drift apart. And the channel is chosen at emission from what the repository already carries and what the team's coding agent loads today — measured, never assumed, because agents change which instruction files they load from release to release.
...
Each product names a **Decider** (config field) who approves spec patches, accepts flags, and can waive walkthroughs; removal or absence falls back to the Owner automatically — handover never blocks on a missing human. A Decider settles a handover-blocking gap whatever their workspace role, so a Developer the product names decides for it.

**The appointment does not override the table above.** The Decider sentence says what a Decider *does*; the table says what a role *may do*, and where they disagree the table wins — a per-product config field must not silently grant a workspace-level write power. The one case where they disagree is the Viewer: read-only means read-only, and a Viewer named as Decider settles nothing. That configuration is a mistake the product **refuses at assignment time** — a Viewer cannot be named Decider, and demoting a member to Viewer clears every Decider field naming them — rather than one it honours and then declines to act on. The fallback above covers a Decider who is *absent*; a Decider who is present and powerless is a silently broken product. Invited members skip onboarding entirely: invite link → Google/Apple/code → land directly on the thing they were invited to, under a minute, zero setup screens.
...
**v1.1:** Linear and Jira, then Azure DevOps; Confluence/Coda/tl;dv; librarian-mode replies inside Notion comment threads; Figma auto-fix (opt-in write-back); confidence cascade + batch-API scoring + full golden harness; runtime hook configs that hard-enforce the ticket fence in Claude Code against the files the builder declares at plan time and returns in the report-back — aenima never sees the code, so the pack asks for the list and never supplies one; additional languages on demand.
...
Nested AGENTS.md per area (backend/, app/) carry area-specific conventions only. No mirror — delivery follows §9's rules.
...
| pack | — | |
| ceremony | — | |
| walkthrough | test | §1 law 6 sets the walkthrough against a test |
| stage | — | |
