...
- `validatePack` holds the base checks to exactly `RUBRIC_TOTAL`, so a new check takes its points from an existing one and a rubric edit cannot move the total under a stored run. — `docs/log/T2.1.md`
