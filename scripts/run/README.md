...
once per API message with cache reads and writes excluded, the outcome from the last Status
the run wrote on its task, and the reviewer's findings counted from its replies. A session
that was not a `/ticket` writes no row.
